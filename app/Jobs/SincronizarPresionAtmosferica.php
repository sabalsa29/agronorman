...
use App\Models\ParcelaErrorPresion;
use Carbon\Carbon;
...
class SincronizarPresionAtmosferica implements ShouldQueue
{
    ...
    {
        Log::info('DEBUG: Entrando a handle() de SincronizarPresionAtmosferica');
        try {
            // Obtener parcelas que tienen coordenadas y NO están en la tabla de errores
            $parcelasConError = ParcelaErrorPresion::activas()->pluck('parcela_id')->toArray();
            ...
            Log::error('DEBUG: Error general en handle() de SincronizarPresionAtmosferica: ' . $e->getMessage());
            throw $e;
        }
    }
    ...
    private function procesarParcela(Parcelas $parcela): void
    {
        try {
            ...
            Log::info('DEBUG: Datos actuales obtenidos para parcela ' . $parcela->id . ', presión: ' . ($datosActuales['pressure'] ?? 0));
        ...
        if (isset($data['list'])) {
            ...
            foreach ($registrosLimitados as $item) {
                ...
                $datos[] = [
                    ...
                    'grnd_level' => $item['main']['grnd_level'] ?? null,
                    'datos_raw' => json_encode([
    ...
    private function guardarDatos(Parcelas $parcela, ZonaManejos $zonaManejo, array $datos, string $tipoDato): int
    {
        ...
        foreach ($datos as $dato) {
            ...
            $existe = PresionAtmosferica::where('parcela_id', $parcela->id)
                ->where('fecha_hora_dato', $dato['fecha_hora_dato'])
                ->where('tipo_dato', $tipoDato)
                ->exists();

            if (!$existe) {
                PresionAtmosferica::create([
                    ...
                    'sea_level' => $dato['sea_level'],
                    'grnd_level' => $dato['grnd_level'],
                    'tipo_dato' => $tipoDato,
                    'fuente' => 'openweather',
    ...
    private function guardarErrorParcela(Parcelas $parcela, string $errorMensaje): void
    {
        try {
            ...
            $errorExistente = ParcelaErrorPresion::where('parcela_id', $parcela->id)->first();
    ...
    {
        Log::error('Job SincronizarPresionAtmosferica falló: ' . $exception->getMessage());
    }
}
