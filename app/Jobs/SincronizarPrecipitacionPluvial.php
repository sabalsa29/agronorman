...
use App\Models\ParcelaErrorPrecipitacion;
use Carbon\Carbon;
...
class SincronizarPrecipitacionPluvial implements ShouldQueue
{
    ...
    {
        Log::info('DEBUG: Entrando a handle() de SincronizarPrecipitacionPluvial');
        try {
            // Obtener parcelas que tienen coordenadas y NO están en la tabla de errores
            $parcelasConError = ParcelaErrorPrecipitacion::activas()->pluck('parcela_id')->toArray();
            ...
            Log::error('DEBUG: Error general en handle() de SincronizarPrecipitacionPluvial: ' . $e->getMessage());
            throw $e;
        }
    }
    ...
    private function procesarParcela(Parcelas $parcela): void
    {
        try {
            ...
            Log::info('DEBUG: Datos actuales obtenidos para parcela ' . $parcela->id . ', precipitación: ' . ($datosActuales['precipitacion_mm'] ?? 0));
    ...
    private function obtenerDatosActuales(Parcelas $parcela): array
    {
        ...
        return [
            ...
            'precipitacion_mm' => $data['rain']['1h'] ?? 0,
            'precipitacion_probabilidad' => 0,
            'datos_raw' => json_encode(['temp' => $data['main']['temp'] ?? 0, 'humidity' => $data['main']['humidity'] ?? 0])
        ];
    }
        ...
        if (isset($data['list'])) {
            ...
            foreach ($registrosLimitados as $item) {
                ...
                $datos[] = [
                    ...
                    'precipitacion_mm' => $item['rain']['3h'] ?? 0,
                    'precipitacion_probabilidad' => $item['pop'] * 100,
                    'datos_raw' => json_encode(['temp' => $item['main']['temp'] ?? 0, 'humidity' => $item['main']['humidity'] ?? 0])
                ];
            }
        }
    ...
    private function guardarDatos(Parcelas $parcela, ZonaManejos $zonaManejo, array $datos, string $tipoDato): int
    {
        ...
        foreach ($datos as $dato) {
            ...
            $existe = PrecipitacionPluvial::where('parcela_id', $parcela->id)
                ->where('fecha_hora_dato', $dato['fecha_hora_dato'])
                ->where('tipo_dato', $tipoDato)
                ->exists();

            if (!$existe) {
                PrecipitacionPluvial::create([
                    ...
                    'precipitacion_probabilidad' => $dato['precipitacion_probabilidad'],
                    'tipo_dato' => $tipoDato,
                    'fuente' => 'openweather',
    ...
    private function guardarErrorParcela(Parcelas $parcela, string $errorMensaje): void
    {
        try {
            ...
            $errorExistente = ParcelaErrorPrecipitacion::where('parcela_id', $parcela->id)->first();
            ...
            if ($errorExistente) {
                ...
                ParcelaErrorPrecipitacion::create([
                    'parcela_id' => $parcela->id,
    ...
    {
        Log::error('Job SincronizarPrecipitacionPluvial falló: ' . $exception->getMessage());
    }
}
