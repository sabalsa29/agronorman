console.log('estres_potasio.js cargado y ejecutándose');
...
window.EchartsColumnsWaterfallsPotasio = (function () {
    var columns_stacked_potasio = null;

    function updateData(data) {
        if (!columns_stacked_potasio) {
            var el = document.getElementById('columns_stacked_potasio');
            if (!el) {
                console.error('No se encontró el elemento columns_stacked_potasio');
                return;
            }
            columns_stacked_potasio = echarts.init(el);
        }

        // Mostrar loader nativo de ECharts
        columns_stacked_potasio.showLoading();
        ...
        if (!data || !data.datos_agrupados || !Array.isArray(data.datos_agrupados)) {
            ...
            columns_stacked_potasio.hideLoading();
            return;
        }
        ...
        columns_stacked_potasio.setOption(option);
        columns_stacked_potasio.hideLoading();

        // Actualizar título con información del tooltip
        actualizarTituloEstresPotasio(cumuloReal, data.total_horas_periodo);
    }

    function actualizarTituloEstresPotasio(cumuloReal, totalAcumulado) {
        var infoElement = document.getElementById('info_estres_potasio');
        if (!infoElement) {
            console.warn('No se encontró el elemento info_estres_potasio');
            return;
        }
