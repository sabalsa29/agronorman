console.log('estres_velocidad_viento.js cargado y ejecutándose');
...
window.EchartsColumnsWaterfallsVelocidadViento = (function () {
    var columns_stacked_velocidad_viento = null;

    function updateData(data) {
        if (!columns_stacked_velocidad_viento) {
            var el = document.getElementById('columns_stacked_velocidad_viento');
            if (!el) {
                console.error('No se encontró el elemento columns_stacked_velocidad_viento');
                return;
            }
            columns_stacked_velocidad_viento = echarts.init(el);
        }

        // Mostrar loader nativo de ECharts
        columns_stacked_velocidad_viento.showLoading();
        ...
        if (!data || !data.datos_agrupados || !Array.isArray(data.datos_agrupados)) {
            ...
            columns_stacked_velocidad_viento.hideLoading();
            return;
        }
        ...
        columns_stacked_velocidad_viento.setOption(option);
        columns_stacked_velocidad_viento.hideLoading();

        // Actualizar título con información del tooltip
        actualizarTituloEstresVelocidadViento(cumuloReal, data.total_horas_periodo);
    }

    function actualizarTituloEstresVelocidadViento(cumuloReal, totalAcumulado) {
        var infoElement = document.getElementById('info_estres_velocidad_viento');
        if (!infoElement) {
            console.warn('No se encontró el elemento info_estres_velocidad_viento');
            return;
        }
