console.log('estres_phos.js cargado y ejecutándose');
...
window.EchartsColumnsWaterfallsPhos = (function () {
    var columns_stacked_phos = null;

    function updateData(data) {
        if (!columns_stacked_phos) {
            var el = document.getElementById('columns_stacked_phos');
            if (!el) {
                console.error('No se encontró el elemento columns_stacked_phos');
                return;
            }
            columns_stacked_phos = echarts.init(el);
        }

        // Mostrar loader nativo de ECharts
        columns_stacked_phos.showLoading();
        ...
        if (!data || !data.datos_agrupados || !Array.isArray(data.datos_agrupados)) {
            ...
            columns_stacked_phos.hideLoading();
            return;
        }
        ...
        columns_stacked_phos.setOption(option);
        columns_stacked_phos.hideLoading();

        // Actualizar título con información del tooltip
        actualizarTituloEstresFosforo(cumuloReal, data.total_horas_periodo);
    }

    function actualizarTituloEstresFosforo(cumuloReal, totalAcumulado) {
        var infoElement = document.getElementById('info_estres_fosforo');
        if (!infoElement) {
            console.warn('No se encontró el elemento info_estres_fosforo');
            return;
        }
