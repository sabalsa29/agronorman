console.log('estres_conductividad_electrica.js cargado y ejecutándose');
...
window.EchartsColumnsWaterfallsConductividadElectrica = (function () {
    var columns_stacked_conductividad_electrica = null;

    function updateData(data) {
        if (!columns_stacked_conductividad_electrica) {
            var el = document.getElementById('columns_stacked_conductividad_electrica');
            if (!el) {
                console.error('No se encontró el elemento columns_stacked_conductividad_electrica');
                return;
            }
            columns_stacked_conductividad_electrica = echarts.init(el);
        }

        // Mostrar loader nativo de ECharts
        columns_stacked_conductividad_electrica.showLoading();
        ...
        if (!data || !data.datos_agrupados || !Array.isArray(data.datos_agrupados)) {
            ...
            columns_stacked_conductividad_electrica.hideLoading();
            return;
        }
        ...
        columns_stacked_conductividad_electrica.setOption(option);
        columns_stacked_conductividad_electrica.hideLoading();

        // Actualizar título con información del tooltip
        actualizarTituloEstresConductividadElectrica(cumuloReal, data.total_horas_periodo);
    }

    function actualizarTituloEstresConductividadElectrica(cumuloReal, totalAcumulado) {
        var infoElement = document.getElementById('info_estres_conductividad_electrica');
        if (!infoElement) {
            console.warn('No se encontró el elemento info_estres_conductividad_electrica');
            return;
        }
