console.log('estres_presion_atmosferica.js cargado y ejecutándose');
...
window.EchartsColumnsWaterfallsPresionAtmosferica = (function () {
    var columns_stacked_presion_atmosferica = null;

    function updateData(data) {
        if (!columns_stacked_presion_atmosferica) {
            var el = document.getElementById('columns_stacked_presion_atmosferica');
            if (!el) {
                console.error('No se encontró el elemento columns_stacked_presion_atmosferica');
                return;
            }
            columns_stacked_presion_atmosferica = echarts.init(el);
        }

        // Mostrar loader nativo de ECharts
        columns_stacked_presion_atmosferica.showLoading();
        ...
        if (!data || !data.datos_agrupados || !Array.isArray(data.datos_agrupados)) {
            ...
            columns_stacked_presion_atmosferica.hideLoading();
            return;
        }
        ...
        columns_stacked_presion_atmosferica.setOption(option);
        columns_stacked_presion_atmosferica.hideLoading();

        // Actualizar título con información del tooltip
        actualizarTituloEstresPresionAtmosferica(cumuloReal, data.total_horas_periodo);
    }

    function actualizarTituloEstresPresionAtmosferica(cumuloReal, totalAcumulado) {
        var infoElement = document.getElementById('info_estres_presion_atmosferica');
        if (!infoElement) {
            console.warn('No se encontró el elemento info_estres_presion_atmosferica');
            return;
        }
