...
window.EchartsColumnsWaterfallsPronosticoHumedadRelativa = (function () {
    var columns_stacked_pronostico_humedad_relativa = null;

    function updatePronosticoDataHumedadRelativa(data) {
        if (!columns_stacked_pronostico_humedad_relativa) {
            var el = document.getElementById('columns_stacked_pronostico_humedad_relativa');
            if (!el) {
                console.error('No se encontró el elemento columns_stacked_pronostico_humedad_relativa');
                return;
            }
            columns_stacked_pronostico_humedad_relativa = echarts.init(el);
        }

        // Mostrar loader nativo de ECharts
        columns_stacked_pronostico_humedad_relativa.showLoading();
        ...
        if (!data || !data.datos_agrupados || !Array.isArray(data.datos_agrupados)) {
            ...
            columns_stacked_pronostico_humedad_relativa.hideLoading();
            return;
        }
        ...
        columns_stacked_pronostico_humedad_relativa.setOption(option);
        columns_stacked_pronostico_humedad_relativa.hideLoading();
    }
    ...
    return {
        init: init,
        updatePronosticoDataHumedadRelativa: updatePronosticoDataHumedadRelativa
    };
})(); 