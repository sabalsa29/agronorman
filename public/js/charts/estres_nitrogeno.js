console.log('estres_nitrogeno.js cargado y ejecutándose');
...
window.EchartsColumnsWaterfallsNitrogeno = (function () {
    var columns_stacked_nitrogeno = null;

    function updateData(data) {
        if (!columns_stacked_nitrogeno) {
            var el = document.getElementById('columns_stacked_nitrogeno');
            if (!el) {
                console.error('No se encontró el elemento columns_stacked_nitrogeno');
                return;
            }
            columns_stacked_nitrogeno = echarts.init(el);
        }

        // Mostrar loader nativo de ECharts
        columns_stacked_nitrogeno.showLoading();
        ...
        if (!data || !data.datos_agrupados || !Array.isArray(data.datos_agrupados)) {
            ...
            columns_stacked_nitrogeno.hideLoading();
            return;
        }
        ...
        columns_stacked_nitrogeno.setOption(option);
        columns_stacked_nitrogeno.hideLoading();

        // Actualizar título con información del tooltip
        actualizarTituloEstresNitrogeno(cumuloReal, data.total_horas_periodo);
    }

    function actualizarTituloEstresNitrogeno(cumuloReal, totalAcumulado) {
        var infoElement = document.getElementById('info_estres_nitrogeno');
        if (!infoElement) {
            console.warn('No se encontró el elemento info_estres_nitrogeno');
            return;
        }
