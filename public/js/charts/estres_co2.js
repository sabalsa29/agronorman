console.log('estres_co2.js cargado y ejecutándose');
...
window.EchartsColumnsWaterfallsCO2 = (function () {
    var columns_stacked_co2 = null;

    function updateData(data) {
        if (!columns_stacked_co2) {
            var el = document.getElementById('columns_stacked_co2');
            if (!el) {
                console.error('No se encontró el elemento columns_stacked_co2');
                return;
            }
            columns_stacked_co2 = echarts.init(el);
        }

        // Mostrar loader nativo de ECharts
        columns_stacked_co2.showLoading();
        ...
        if (!data || !data.datos_agrupados || !Array.isArray(data.datos_agrupados)) {
            ...
            columns_stacked_co2.hideLoading();
            return;
        }
        ...
        columns_stacked_co2.setOption(option);
        columns_stacked_co2.hideLoading();

        // Actualizar título con información del tooltip
        actualizarTituloEstresCO2(cumuloReal, data.total_horas_periodo);
    }

    function actualizarTituloEstresCO2(cumuloReal, totalAcumulado) {
        var infoElement = document.getElementById('info_estres_co2');
        if (!infoElement) {
            console.warn('No se encontró el elemento info_estres_co2');
            return;
        }
