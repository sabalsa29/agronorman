console.log('estres_ph.js cargado y ejecutándose');
...
window.EchartsColumnsWaterfallsPH = (function () {
    var columns_stacked_ph = null;

    function updateData(data) {
        if (!columns_stacked_ph) {
            var el = document.getElementById('columns_stacked_ph');
            if (!el) {
                console.error('No se encontró el elemento columns_stacked_ph');
                return;
            }
            columns_stacked_ph = echarts.init(el);
        }

        // Mostrar loader nativo de ECharts
        columns_stacked_ph.showLoading();
        ...
        if (!data || !data.datos_agrupados || !Array.isArray(data.datos_agrupados)) {
            ...
            columns_stacked_ph.hideLoading();
            return;
        }
        ...
        columns_stacked_ph.setOption(option);
        columns_stacked_ph.hideLoading();

        // Actualizar título con información del tooltip
        actualizarTituloEstresPH(cumuloReal, data.total_horas_periodo);
    }

    function actualizarTituloEstresPH(cumuloReal, totalAcumulado) {
        var infoElement = document.getElementById('info_estres_ph');
        if (!infoElement) {
            console.warn('No se encontró el elemento info_estres_ph');
            return;
        }
