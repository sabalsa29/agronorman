console.log('estres_temperatura.js cargado y ejecutándose');
...
window.EchartsColumnsWaterfallsTemperatura = (function () {
    var columns_stacked_temperatura = null;

    function updateData(data) {
        if (!columns_stacked_temperatura) {
            var el = document.getElementById('columns_stacked_temperatura');
            if (!el) {
                console.error('No se encontró el elemento columns_stacked_temperatura');
                return;
            }
            columns_stacked_temperatura = echarts.init(el);
        }

        // Mostrar loader nativo de ECharts
        columns_stacked_temperatura.showLoading();
        ...
        if (!data || !data.datos_agrupados || !Array.isArray(data.datos_agrupados)) {
            ...
            columns_stacked_temperatura.hideLoading();
            return;
        }
        ...
        columns_stacked_temperatura.setOption(option);
        columns_stacked_temperatura.hideLoading();

        // Actualizar título con información del tooltip
        actualizarTituloEstresTemperatura(cumuloReal, data.total_horas_periodo);
    }

    function actualizarTituloEstresTemperatura(cumuloReal, totalAcumulado) {
        var infoElement = document.getElementById('info_estres_temperatura');
        if (!infoElement) {
            console.warn('No se encontró el elemento info_estres_temperatura');
            return;
        }
