console.log('estres_pronostico_velocidad_viento.js cargado y ejecutándose');
...
window.EchartsColumnsWaterfallsPronosticoVelocidadViento = (function () {
    var columns_stacked_velocidad_viento_pronostico = null;

    function updatePronosticoDataVelocidadViento(data) {
        if (!columns_stacked_velocidad_viento_pronostico) {
            var el = document.getElementById('columns_stacked_velocidad_viento_pronostico');
            if (!el) {
                console.error('No se encontró el elemento columns_stacked_velocidad_viento_pronostico');
                return;
            }
            columns_stacked_velocidad_viento_pronostico = echarts.init(el);
        }

        // Mostrar loader nativo de ECharts
        columns_stacked_velocidad_viento_pronostico.showLoading();
        ...
        if (!data || !data.datos_agrupados || !Array.isArray(data.datos_agrupados)) {
            ...
            columns_stacked_velocidad_viento_pronostico.hideLoading();
            return;
        }
        ...
        columns_stacked_velocidad_viento_pronostico.setOption(option);
        columns_stacked_velocidad_viento_pronostico.hideLoading();
    }
    ...
    return {
        init: init,
        updatePronosticoDataVelocidadViento: updatePronosticoDataVelocidadViento
    };
})(); 