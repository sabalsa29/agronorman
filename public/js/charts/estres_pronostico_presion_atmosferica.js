console.log('estres_pronostico_presion_atmosferica.js cargado y ejecutándose');
...
window.EchartsColumnsWaterfallsPronosticoPresionAtmosferica = (function () {
    var columns_stacked_presion_atmosferica_pronostico = null;

    function updatePronosticoDataPresionAtmosferica(data) {
        if (!columns_stacked_presion_atmosferica_pronostico) {
            var el = document.getElementById('columns_stacked_presion_atmosferica_pronostico');
            if (!el) {
                console.error('No se encontró el elemento columns_stacked_presion_atmosferica_pronostico');
                return;
            }
            columns_stacked_presion_atmosferica_pronostico = echarts.init(el);
        }

        // Mostrar loader nativo de ECharts
        columns_stacked_presion_atmosferica_pronostico.showLoading();
        ...
        if (!data || !data.datos_agrupados || !Array.isArray(data.datos_agrupados)) {
            ...
            columns_stacked_presion_atmosferica_pronostico.hideLoading();
            return;
        }
        ...
        var cumuloReal = data.cumulo_real && Array.isArray(data.cumulo_real) ? data.cumulo_real : [];
            ...
            return {
                ...
                label: {
                    ...
                    formatter: function (params) {
                        ...
                        // Usar los datos específicos del día actual
                        var diaIndex = params.dataIndex;
                        if (cumuloReal && cumuloReal[diaIndex] && key && cumuloReal[diaIndex][key] !== undefined) {
        ...
        var option = {
            ...
            legend: {
                data: categorias.map(function (cat) {
                    ...
                    // Calcular totales acumulados de todos los días
                    var totalHoras = 0;
                    var totalCategoria = 0;

                    if (cumuloReal && cumuloReal.length > 0) {
                        cumuloReal.forEach(function (dia) {
                            if (dia[cat.key] && dia[cat.key].h !== undefined) {
                                totalCategoria += dia[cat.key].h;
                                totalHoras += Object.values(dia).reduce(function (sum, item) {
                                    return sum + (item.h || 0);
                                }, 0);
                            }
                        });
                        ...
                        if (totalHoras > 0) {
                            porcentaje = ` (${Math.round((totalCategoria / totalHoras) * 100)}%)`;
                        }
                    }
        ...
        columns_stacked_presion_atmosferica_pronostico.setOption(option);
        columns_stacked_presion_atmosferica_pronostico.hideLoading();
    }
    ...
    return {
        init: init,
        updatePronosticoDataPresionAtmosferica: updatePronosticoDataPresionAtmosferica
    };
})(); 