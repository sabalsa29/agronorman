console.log('estres_temperatura_suelo.js cargado y ejecutándose');
...
window.EchartsColumnsWaterfallsSuelo = (function () {
    var columns_stacked_suelo = null;

    function updateData(data) {
        if (!columns_stacked_suelo) {
            var el = document.getElementById('columns_stacked_suelo');
            if (!el) {
                console.error('No se encontró el elemento columns_stacked_suelo');
                return;
            }
            columns_stacked_suelo = echarts.init(el);
        }

        // Mostrar loader nativo de ECharts
        columns_stacked_suelo.showLoading();
        ...
        if (!data || !data.datos_agrupados || !Array.isArray(data.datos_agrupados)) {
            ...
            columns_stacked_suelo.hideLoading();
            return;
        }
        ...
        columns_stacked_suelo.setOption(option);
        columns_stacked_suelo.hideLoading();

        // Actualizar título con información del tooltip
        actualizarTituloEstresTemperaturaSuelo(cumuloReal, data.total_horas_periodo);
    }

    function actualizarTituloEstresTemperaturaSuelo(cumuloReal, totalAcumulado) {
        var infoElement = document.getElementById('info_estres_temperatura_suelo');
        if (!infoElement) {
            console.warn('No se encontró el elemento info_estres_temperatura_suelo');
            return;
        }
