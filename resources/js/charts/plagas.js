...
// After GaugeCustomFactory definition, initialize gauges for each plaga
document.addEventListener('DOMContentLoaded', function () {
    if (!window.plagasIds || !window.semaforosData) {
        console.warn('plagasIds or semaforosData not defined');
        return;
    }
    window.plagasIds.forEach(function (id) {
        ...
        if (!semaforo) {
            console.warn('No semaforo data for plaga ID', id);
            return;
        }
        var containerId = 'gauge_custom_' + id;
        GaugeCustomFactory.create(
            containerId,
            semaforo.porcentaje,
            semaforo.etapa,
            semaforo.color
