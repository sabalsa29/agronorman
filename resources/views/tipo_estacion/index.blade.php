...
@if (Auth::check() && Auth::user()->canCreate('estaciones.tipo_estacion'))
    @section('ruta_create', route('tipo_estacion.create'))
@endif
@section('styles')
    <!-- Theme JS files -->
    ...
    <div class="card">
        ...
        <table class="table datatable-responsive">
            <thead>
                <tr>
                    <th>Nombre</th>
                    <th class="text-center">Tipo Nasa</th>
                ...
                @forelse ($list as $row)
                    <tr>
                        ...
                        <td class="text-center">
                            <div class="list-icons">
                                @if (Auth::check() && Auth::user()->canEdit('estaciones.tipo_estacion'))
                                    <a href="{{ route('tipo_estacion.edit', [$row]) }}"
                                        class="list-icons-item text-primary-600"><i class="icon-pencil7"></i></a>
                                @endif
                                @if (Auth::check() && Auth::user()->canDelete('estaciones.tipo_estacion'))
                                    <form action="{{ route('tipo_estacion.destroy', $row) }}" method="POST"
