<div>
    ...
    <div class="card shadow-lg border-0 mt-4">
        <div class="card-header header-elements-inline bg-gradient-secondary text-white">
            <div class="d-flex flex-column align-items-start">
                <h3 class="card-title mb-0" id="titulo_estres_co2">
                    <i class="icon-stats-bars mr-2"></i>
                    Análisis de Estrés por CO₂
                </h3>
                <div id="info_estres_co2" class="estres-info mt-2" style="font-size: 0.875rem; opacity: 0.9;"></div>
            </div>
            <div class="header-elements">
                <div class="list-icons">
                    <a class="list-icons-item" data-action="collapse"></a>
        ...
        <div class="card-body">
            <div class="chart-container">
                <div class="chart has-fixed-height" id="columns_stacked_co2"></div>
            </div>
        </div>
    </div>
    <!-- /gráfica de estrés -->
    ...
    <script>
        ...
        function cargaGraficaEstresCO2(zona, periodo, startDate, endDate) {
            if (typeof window.EchartsColumnsWaterfallsCO2 === 'undefined') {
                console.error('EchartsColumnsWaterfallsCO2 no está disponible. Esperando a que se cargue...');
                return;
            }
            const params = {
                estacion_id: zona,
            ...
            console.log('Loading CO2 stress data with params:', params);
            return axios.get("{{ route('home.grafica_estres') }}", {
                    params: params
                })
                .then(function(response) {
                    console.log('CO2 stress data response:', response.data);
                    window.EchartsColumnsWaterfallsCO2.updateData(response.data, 'columns_stacked_co2');
                })
                .catch(function(error) {
                    console.error('Error loading CO2 stress data:', error);
                    if (error.response) {
                        console.error('Error response:', error.response.data);
                    }
                });
        ...
        function cargarGraficasCO2() {
            ...
            // Inicializar la gráfica de estrés de CO2
            if (window.EchartsColumnsWaterfallsCO2 && window.EchartsColumnsWaterfallsCO2.init) {
                window.EchartsColumnsWaterfallsCO2.init('columns_stacked_co2');
            }
            // Cargar datos históricos de estrés de CO2
            cargaGraficaEstresCO2(zona, periodo, startDate, endDate);
        }
        window.cargarGraficasCO2 = cargarGraficasCO2;
        window.cargaGraficaEstresCO2 = cargaGraficaEstresCO2;
    </script>
</div>
