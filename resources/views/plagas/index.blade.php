...
@section('ruta_create', route('plaga.create'))
@section('styles')
    <!-- Theme JS files -->
...
@section('content')
    ...
    <div class="card">
        ...
        <table class="table datatable-responsive">
            ...
            <tbody>
                @forelse ($list['plagues'] as $row)
                    <tr>
                        <td>{{ $row->nombre ?? 'Sin nombre' }}</td>
                        <td>{{ $row->umbral_min ?? 'Sin umbral' }}</td>
                        <td>{{ $row->umbral_max ?? 'Sin umbral' }}</td>
                        <td>{{ $row->nombres_cultivos }}</td>
                        <td class="text-center">
                            <div class="list-icons">
                                <a href="{{ route('plaga.edit', [$row]) }}" class="list-icons-item text-primary-600"><i
                                        class="icon-pencil7"></i></a>
                                <form action="{{ route('plaga.destroy', $row) }}" method="POST"
