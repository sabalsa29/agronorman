...
@section('ruta_home', route('almacenes.index'))
@if (Auth::check() && Auth::user()->canCreate('estaciones.almacenes'))
    @section('ruta_create', route('almacenes.create'))
@endif
@section('styles')
    <!-- Theme JS files -->
                ...
                @forelse ($list as $row)
                    <tr>
                        ...
                        <td class="text-center">
                            <div class="list-icons">
                                @if (Auth::check() && Auth::user()->canEdit('estaciones.almacenes'))
                                    <a href="{{ route('almacenes.edit', [$row]) }}" class="list-icons-item text-primary-600"><i
                                            class="icon-pencil7"></i></a>
                                @endif
                                @if (Auth::check() && Auth::user()->canDelete('estaciones.almacenes'))
                                    <form action="{{ route('almacenes.destroy', $row) }}" method="POST"
