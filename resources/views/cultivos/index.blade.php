...
@section('ruta_create', route('cultivos.create'))
@section('styles')
    <!-- Theme JS files -->
...
@section('content')
    ...
    <div class="card">
        ...
        <table class="table datatable-responsive">
            ...
            <tbody>
                @forelse ($list['cultivos'] as $row)
                    <tr>
                        <td>{{ $row->nombre ?? 'Sin nombre' }}</td>
                        <td class="text-center">
                            <div class="list-icons">
                                <a href="{{ route('cultivos.edit', [$row]) }}" class="list-icons-item text-primary-600"><i
                                        class="icon-pencil7"></i></a>
                                <form action="{{ route('cultivos.destroy', $row) }}" method="POST"
                                    class="d-inline delete-form">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" class="btn p-0 pl-3 pr-3 border-0 bg-transparent delete-button"
                                        data-name="{{ $row->nombre }}" title="Eliminar">
                                        <i class="icon-trash text-danger-600"></i>
                                    </button>
                                </form>
                                <a href="{{ route('tipo_cultivos.index', $row) }}">
                                    <button type="button" class="btn bg-green-800 btn-ladda btn-ladda-progress"
                                        data-style="slide-left">
                                        <span class="ladda-label">Tipo de cultivos</span>
                                    </button>
                                </a>
                            </div>
                        </td>
