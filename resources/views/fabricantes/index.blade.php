...
@if (Auth::check() && Auth::user()->canCreate('estaciones.fabricantes'))
    @section('ruta_create', route('fabricantes.create'))
@endif
@section('styles')
    <!-- Theme JS files -->
...
@section('content')
    ...
    <div class="card">
        ...
        <table class="table datatable-responsive">
            ...
            <tbody>
                @forelse ($list['fabricantes'] as $row)
                    <tr>
                        ...
                        <td class="text-center">
                            <div class="list-icons">
                                @if (Auth::check() && Auth::user()->canEdit('estaciones.fabricantes'))
                                    <a href="{{ route('fabricantes.edit', [$row]) }}"
                                        class="list-icons-item text-primary-600"><i class="icon-pencil7"></i></a>
                                @endif
                                @if (Auth::check() && Auth::user()->canDelete('estaciones.fabricantes'))
                                    <form action="{{ route('fabricantes.destroy', $row) }}" method="POST"
