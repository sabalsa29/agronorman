...
@section('ruta_create', route('usuarios.create', ['id' => $cliente_id]))
@section('styles')
    <!-- Theme JS files -->
                ...
                @forelse ($list as $row)
                    <tr>
                        ...
                        <td class="text-center">
                            <div class="list-icons">
                                <a href="{{ route('usuarios.edit', ['id' => $cliente_id, $row]) }}"
                                    class="list-icons-item text-primary-600"><i class="icon-pencil7"></i></a>
                                <form action="{{ route('usuarios.destroy', ['id' => $cliente_id, $row]) }}" method="POST"
                                ...
                                @if (Auth::check() && Auth::user()->isSuperAdmin())
                                    <a href="{{ route('usuarios.permissions', ['id' => $cliente_id, $row]) }}"
                                        class="list-icons-item text-warning-600" title="Permisos">
                                        <i class="icon-lock2 ml-3"></i>
                                    </a>
                                @endif
                            </div>
                        </td>
