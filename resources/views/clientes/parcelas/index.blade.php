...
@section('ruta_create', route('parcelas.create', ['id' => $cliente_id]))
@section('styles')
    <!-- Theme JS files -->
                ...
                @forelse ($list as $row)
                    <tr>
                        ...
                        <td class="text-center">
                            <div class="list-icons">
                                <a href="{{ route('parcelas.edit', ['id' => $cliente_id, 'parcela' => $row]) }}"
                                    class="list-icons-item text-primary-600"><i class="icon-pencil7"></i></a>
                                <form action="{{ route('parcelas.destroy', ['id' => $cliente_id, 'parcela' => $row]) }}"
                                ...
                                <a href="{{ route('zona_manejo.index', ['id' => $cliente_id, 'parcela_id' => $row]) }}">
                                    <button type="button" class="btn bg-green-800 btn-ladda btn-ladda-progress"
                                        data-style="slide-left">
                                        <span class="ladda-label">Zonas de manejo</span>
                                    </button>
                                </a>
                            </div>
                        </td>
