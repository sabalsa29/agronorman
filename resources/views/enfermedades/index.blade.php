    ...
    <div class="card">
        ...
        <table class="table datatable-responsive">
            <thead>
                <tr>
                    <th>Nombre</th>
                    <th>Tipos de Cultivo</th>
                    <th class="text-center">Actions</th>
                </tr>
            </thead>
            <tbody>
                @forelse ($list['enfermedades'] as $row)
                    <tr>
                        <td>{{ $row->nombre ?? 'Sin nombre' }}</td>
                        <td>{{ $row->especies_list }}</td>
                        <td class="text-center">
                            <div class="list-icons">
                                <a href="{{ route('enfermedades.edit', [$row]) }}"
                                    class="list-icons-item text-primary-600"><i class="icon-pencil7"></i></a>
                                <form action="{{ route('enfermedades.destroy', $row) }}" method="POST"
                                ...
                                <a href="{{ route('enfermedades.cultivos.index', $row) }}">
                                    <button type="button" class="btn bg-green-800 btn-ladda btn-ladda-progress"
                                        data-style="slide-left">
                                        <span class="ladda-label">Tipos de Cultivo</span>
                                    </button>
                                </a>
                            </div>
                        </td>
